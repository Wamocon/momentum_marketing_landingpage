/* ==============================================
   Momentum Marketing App — Landing Page JS
...
(function () {
    ...
    /* ── 7. HERO PARTICLES (Momentum Red palette) ── */
    function initParticles() {
        ...
        function create() {
            ...
            return {
                ...
                // 0=pure red, 40=rose/pink, 80=purple
        ...
        function animate() {
            ...
            for (var i = 0; i < particles.length; i++) {
                ...
                // Red → rose → purple gradient: hue 0=red, 40=rose, 80=purple
                var h = p.hue;
                var r, g, b;
                if (h < 40) {
                    // Red (#c1292e) to Rose (#ef4444)
                    var t = h / 40;
                    r = Math.round(193 + t * 46); // 193→239
                    g = Math.round(41  + t * 27); // 41→68
                    b = Math.round(46  + t * 22); // 46→68
                } else {
                    // Rose (#ef4444) to Purple (#8b5cf6)
                    var t = (h - 40) / 40;
                    r = Math.round(239 - t * 148); // 239→91
                    g = Math.round(68  - t * 22);  // 68→46 (approx: 8b=139 for purple, but keeping reddish)
                    b = Math.round(68  + t * 182); // 68→250 (purple blue)
                    r = Math.round(239 - t * 148);
                    g = Math.round(68  + t * 24);  // keep greenish toned
                    b = Math.round(68  + t * 180);
                }
    ...
    function initDemoIframe() {
        ...
        var fallback = document.getElementById('iframeFallback');
        if (!iframe || !fallback) return;

        // After load, try to read contentDocument — if blocked (X-Frame-Options / CSP),
        // the browser keeps contentDocument null or throws a SecurityError.
        iframe.addEventListener('load', function () {
            try {
                // If access is denied, this throws or returns null
        ...
        // Also catch network error (no load event fired within 12 s on a visible element)
